...
export class CheckoutPage implements OnInit {
  ...
  addPayment(){
    ...
    swal("Awesome", "You just bought " + this.dataService.cart.length + " awesome dresses", "success");
    console.log("cart", this.dataService.cart);
    this.dataService.orders = this.dataService.cart.map (cart => {
      return {
        product: cart.product,
        amount: 1,
        customer: this.dataService.logged_user,
        order_date: new Date(),
